...
export async function POST(req: Request) {
  ...
  const res = action === "signup" ? signUp(email, password, name) : signIn(email, password);
  ...
  return new Response(JSON.stringify({ ok: true, email: res.email }), {
    headers: {
      "Content-Type": "application/json",
      "Set-Cookie": `${SESSION_COOKIE}=${res.token}; HttpOnly; Path=/; SameSite=Lax; Max-Age=${7 * 24 * 3600}`,
    },
  });
}
