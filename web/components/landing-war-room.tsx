...
const SCRIPT: Beat[] = [
  { t: "agent", agent: "detective", text: "Davis opened problem P-26065 on checkout. Grail shows 1,271 error events and a peak latency of 1,206ms. The payment-service deploy added a 1,200ms delay." },
...
export function LandingWarRoom() {
  ...
  return (
    <div
      ...
      className="overflow-hidden rounded-xl border border-border bg-[#0b0d10] shadow-2xl shadow-black/50"
    >
      {/* window chrome */}
      <div className="flex items-center gap-2 border-b border-border/70 bg-card/60 px-4 py-2.5">
        ...
        <span className="ml-2 flex items-center gap-1.5 text-xs font-medium text-foreground/80">
          <Hexagon className="h-3.5 w-3.5" strokeWidth={2.5} fill="currentColor" /> HiveMind War Room
        </span>
        <span className="ml-auto inline-flex items-center gap-1.5 rounded-full border border-border bg-background/60 px-2 py-0.5 text-[11px] text-muted-foreground">
          <Live /> {paused ? "Paused" : "Replaying a real incident"}
        </span>
      </div>

      <div className="flex h-[560px] sm:h-[620px]">
        {/* sidebar */}
        <aside className="hidden w-48 shrink-0 flex-col border-r border-border/70 bg-card/30 p-3 md:flex">
          ...
          <div className="mt-auto flex items-center gap-2 pt-3 text-[11px] text-muted-foreground">
            <span className="flex h-6 w-6 items-center justify-center rounded-full border border-border bg-background text-[10px]">RS</span>
            6 agents online
          </div>
        </aside>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex h-11 shrink-0 items-center gap-2 border-b border-border/70 px-4">
            ...
            <span className="text-sm font-semibold">inc-checkout</span>
            <span className="rounded-full border border-red-500/40 bg-red-500/10 px-1.5 py-0.5 text-[10px] font-semibold text-red-300">SEV1</span>
            <span className="truncate text-xs text-muted-foreground">checkout latency past the 300ms goal</span>
          </div>

          <div ref={feedRef} className="flex flex-1 flex-col gap-3 overflow-hidden px-4 py-3">
            {/* brief */}
            <div className="shrink-0 rounded-lg border border-border/70 bg-card/40 px-3.5 py-2.5">
              <div className="flex items-center gap-2">
                <AlertTriangle className="h-3.5 w-3.5 text-red-300" strokeWidth={2} />
                <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Incident brief</span>
              </div>
        ...
        <aside className="hidden w-64 shrink-0 flex-col gap-px overflow-hidden border-l border-border/70 bg-card/20 lg:flex">
          <RailBlock title="Impact">
            <div className="text-lg font-semibold text-foreground">$18,397<span className="text-sm font-normal text-muted-foreground">/mo at risk</span></div>
            <div className="mt-0.5 text-xs text-muted-foreground">7 customers · Enterprise, Pro, Free</div>
          </RailBlock>
          ...
          <RailBlock title="The fix">
            {mrShown ? (
              <span className="inline-flex items-center gap-1.5 rounded-md border border-amber-400/30 bg-amber-400/10 px-2 py-1 font-mono text-[11px] text-amber-300">
                <GitMerge className="h-3 w-3" /> MR #52 · merged
              </span>
            ) : (
          ...
          <RailBlock title="Recovery">
            {recovered ? (
              ...
              <span className="text-xs text-muted-foreground">waiting for the fix…</span>
            )}
          </RailBlock>
        </aside>
      </div>
...
function SideGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-3">
      <div className="px-1 pb-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground/70">{title}</div>
...
function SideChannel({ name, active, sev }: { name: string; active?: boolean; sev?: boolean }) {
  return (
    <div className={cn("flex cursor-default items-center gap-1.5 rounded-md px-2 py-1 text-[13px]", active ? "bg-accent/60 text-foreground" : "text-muted-foreground")}>
  ...
  if (beat.t === "approve") {
    return (
      <div className="hm-reveal flex shrink-0 items-center gap-2.5">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-border bg-muted text-foreground">
          <UserRound className="h-3.5 w-3.5" strokeWidth={2} />
        </span>
        <p className="text-[13px] text-foreground/85">
          <span className="font-semibold">You</span> approved the fix:{" "}
          <span className="italic text-muted-foreground">&ldquo;Ship it.&rdquo;</span>
        </p>
      </div>
    );
  }
